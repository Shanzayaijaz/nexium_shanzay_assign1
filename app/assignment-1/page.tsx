...
export default function QuoteGenerator() {
  ...
  // Use only blue shades for badges
  const badgeColors = [
    { bg: 'bg-blue-200', border: 'border-blue-300', text: 'text-blue-800', hover: 'hover:bg-blue-300', icon: 'text-blue-500' },
    { bg: 'bg-blue-100', border: 'border-blue-200', text: 'text-blue-700', hover: 'hover:bg-blue-200', icon: 'text-blue-400' },
    { bg: 'bg-blue-300', border: 'border-blue-400', text: 'text-blue-900', hover: 'hover:bg-blue-400', icon: 'text-blue-600' },
    { bg: 'bg-blue-50', border: 'border-blue-100', text: 'text-blue-600', hover: 'hover:bg-blue-100', icon: 'text-blue-300' },
  ];
    ...
    <div className="min-h-screen w-full max-w-xl mx-auto sm:mt-16 mt-4 sm:p-8 p-2 space-y-6 bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 rounded-2xl shadow-xl flex flex-col justify-center">
      <div className="text-center space-y-2">
        ...
        <motion.div
          ...
          className="w-full max-w-lg mx-auto bg-blue-50/80 rounded-xl shadow p-2 sm:p-4 mt-4 border-2 border-transparent bg-clip-padding"
          style={{ borderImage: 'linear-gradient(90deg, #3b82f6, #60a5fa) 1' }}
        >
            ...
            {availableTopics.map((t, i) => {
              ...
              return (
                <motion.span
                  ...
                  whileHover={{ scale: 1.12, boxShadow: '0 2px 12px #bae6fd' }}
                  className={`flex items-center gap-1 px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold border shadow-sm transition-transform duration-150 cursor-pointer ${color.bg} ${color.text} ${color.border} ${color.hover}`}
                  onClick={() => handleBadgeClick(t)}
                  tabIndex={0}
                  role="button"
                  aria-label={`Select topic ${t}`}
                >
                  <Tag className={`w-3 h-3 ${color.icon}`} /> {t}
                </motion.span>
              );
            })}
          </motion.div>
        ...
        {quotes.map((quote, index) => (
          ...
          >
            <Card className="bg-white border-l-4 border-blue-400 shadow-lg hover:shadow-2xl transition-shadow duration-200 w-full">
              <CardContent className="p-3 sm:p-4 text-sm sm:text-md italic text-blue-800">
